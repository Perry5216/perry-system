...
// Find steps from passes 87+ in project-77 and reset them to pending
...
console.log(`Found ${steps.length} completed steps from contaminated passes to roll back:`);

const resetStmt = db.prepare(`
  UPDATE steps SET status = 'pending', result = NULL, completed_at = NULL 
...
const tx = db.transaction(() => {
  for (const step of steps) {
    ...
    console.log(`  Reset: ${step.id} (${step.label})`);
  }
});
tx();

// Also update the project blob to reflect the reset steps
...
db.close();
console.log('Done. Steps from passes 87-90 reset to pending.');
